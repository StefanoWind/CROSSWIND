...
drs=[]
for s in sources:
   Data[s]=xr.open_dataset(sources[s])
...
plt.pcolor(da.values,cmap='coolwarm')
    
ippr=0
for ppr in pprs:
    idr=0
    for dr in drs: 
        plt.text(ippr+0.5,idr+0.5,f'{str(int(da.loc[dr,ppr].values))}')
        idr+=1
    ippr+=1
plt.xticks(np.arange(len(pprs))+0.5,pprs)
plt.yticks(np.arange(len(drs))+0.5,drs)
plt.xlabel('Pulses per rar (PPR)')
plt.ylabel('Rang gate length Δr [m]')
...
for ppr in pprs:
    idr=0
    for dr in drs:
        ax=plt.subplot(len(drs),len(pprs),idr*len(pprs)+ippr+1)
        for s in sources:
        ...
        plt.text(2500,-5,f'PPR={str(int(ppr))}, Δr={str(int(dr))} m')
        if idr==len(drs)-1:
            plt.xlabel('Range [m]')
        if ippr==0:
            # plt.ylabel(r'Radial wind speed [m s$^{-1}$]')
            plt.ylabel(r'SNR [dB]')
        plt.grid()
        plt.ylim([-30,0])
...
for ppr in pprs:
    ...
    for dr in drs:
        ...
        plt.text(2500,4,f'PPR={str(int(ppr))}, Δr={str(int(dr))} m')
        if idr==len(drs)-1:
            plt.xlabel('Range [m]')
        if ippr==0:
